/**
 * Call To Action Dialog Editor JavaScript
 * Hides secondary CTA link/target fields when secondary CTA text is empty.
 */
(function(document, $, Coral) {
    'use strict';

    var HIDDEN_CLASS = 'cmp-call-to-action--hidden';
    ...
    function initDialog(dialog) {
        if (!dialog.find('.cmp-call-to-action__editor').length) {
            return;
        }
    ...
    $(document).on('foundation-contentloaded', function(event) {
        ...
        if (dialog.length && dialog.find('.cmp-call-to-action__editor').length) {
            setTimeout(function() {
                initDialog(dialog);
            }, 200);
        }
